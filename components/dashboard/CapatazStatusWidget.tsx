...
import { MessageSquare, Signal, Zap, RefreshCw, Smartphone, ShieldCheck, MoreHorizontal } from "lucide-react"
import { motion } from "framer-motion"

export default function CapatazStatusWidget() {
    const [status, setStatus] = useState<'online' | 'pairing' | 'offline'>('online')
            ...
            <div className="relative z-10 flex flex-col h-full">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5">
                                <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${status === 'online' ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.6)]' : 'bg-amber-400'}`}></div>
                                <span className={`text-[10px] uppercase font-black tracking-widest ${status === 'online' ? 'text-emerald-400' : 'text-amber-400'}`}>
                                    {status === 'online' ? 'Instance Active' : 'Connecting...'}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex-1 space-y-6">
                    <div className="flex items-center justify-between p-4 bg-slate-800/40 rounded-2xl border border-slate-700/50">
                        ...
                        <span className="text-[10px] font-mono text-emerald-400 bg-emerald-400/10 px-2 py-1 rounded">52...1234</span>
                    </div>
                ...
                <div className="mt-8 pt-6 border-t border-slate-800/50 flex items-center justify-between gap-4">
                    <button className="flex-1 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold transition-all shadow-lg shadow-emerald-950 flex items-center justify-center gap-2 group">
                        <RefreshCw className="w-3.5 h-3.5 group-hover:rotate-180 transition-transform duration-500" />
                        Refrescar
                    </button>
                    <button className="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-xs font-bold transition-all border border-slate-700 flex items-center gap-2">
